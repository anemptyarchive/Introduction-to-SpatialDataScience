...
dir_path += 'spatial_weight_matrix_by_kNN/' # フォルダを指定
print(dir_path)
...
fig.suptitle('spatial weight matrix: k-NN', fontsize=20)
...
def update(frame_i):
    ...
    for i in range(N):
        adj_bool_vec    = weight_mat[i] > 0.0                     # 隣接区域のフラグ
        mutual_bool_vec = adj_bool_vec & (weight_mat[:, i] > 0.0) # 双方向フラグ
        if i == n:
            adj_idx, = np.where(adj_bool_vec) # 隣接区域のインデックス
        else:
            mutual_idx,   = np.where(mutual_bool_vec)                 # 隣接区域のインデックス(双方向)
            mutual_idx    = mutual_idx[mutual_idx > i]                # 重複を除去
            directed_idx, = np.where(adj_bool_vec != mutual_bool_vec) # 隣接区域のインデックス(一方向)
            adj_idx       = np.hstack([mutual_idx, directed_idx])     # 隣接区域のインデックス
        for j in adj_idx:
            Q_x, Q_y = gdf_target.loc[i, 'centroids'].coords[0] # 各区域の座標
            P_x, P_y = gdf_target.loc[j, 'centroids'].coords[0] # 隣接区域の座標
            mutual_flg = mutual_bool_vec[j] # 双方向フラグ:(ベクトルの書き分け用)
            ax.plot(
                [Q_x, P_x], 
                [Q_y, P_y], 
...
def update(frame_i):
    ...
    for j in adj_idx:
        P_x, P_y = gdf_target.loc[j, 'centroids'].coords[0] # 隣接区域の座標
        mutual_flg = weight_mat[j, area_idx] > 0.0 # 双方向フラグ:(ベクトルの書き分け用)
        ax.plot(
            [O_x, P_x], 
            [O_y, P_y], 
...
# 動画を書出
anim.save(
    filename=dir_path+'weight_mat_k_one.mp4', 
    progress_callback=lambda i, n: print(f'\rframe: {i+1} / {n}', end='', flush=True)
)
...
def update(frame_i):
    ...
    # バンド幅を取得
    adj_idx_lt = [np.where(weight_mat[i] > 0.0)[0] for i in range(N)] # 隣接区域のインデックス
    ...
    for i in range(N):
        ...
        for j in adj_idx:
            ...
            P_x, P_y = gdf_target.loc[j, 'centroids'].coords[0] # 隣接区域の座標
            mutual_flg = weight_mat[j, i] > 0.0 # 双方向フラグ:(ベクトルの書き分け用)
            ax.plot(
                [O_x, P_x], 
                [O_y, P_y], 
...
# 動画を書出
anim.save(
    filename=dir_path+'weight_mat_k_all.mp4', 
    progress_callback=lambda i, n: print(f'\rframe: {i+1} / {n}', end='', flush=True)
)
